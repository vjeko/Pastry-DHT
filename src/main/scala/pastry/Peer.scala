...
class Peer(myID: BigInt) extends Actor with Config {
  
  val logger = Logger(LoggerFactory.getLogger("pastry"))
  val myIDStr = toBase(myID)
  ...
  def handleBootstrap(sender : ActorRef, booststrapPeer: BigInt) = {
    val bootstrapNode = getRef(booststrapPeer)
    val msg = JoinRequest(myID, ListBuffer())
    
    logger.trace(myIDStr + ": " + "Sending " + msg + " to " + toBase(booststrapPeer) )
  ...
  def receive = {
    ...
    // External API:
    case Bootstrap(booststrapPeer) => handleBootstrap(sender, booststrapPeer)
